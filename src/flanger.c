    ...
    sox_sample_t * obuf, size_t * isamp, size_t * osamp)
{
  ...
  *isamp = *osamp = len * channels;
  ...
  /* Switch on the interpolation type outside the per-sample loop
   * for speed (~20% faster than a switch in the middle) */
  ...
  case INTERP_NONE:
	...
	float  delay = f->lfo[(f->lfo_pos + phase) % f->lfo_length];
	size_t int_delay = (size_t)(delay + 0.5f); /* Round to nearest */
  ...
  case INTERP_QUADRATIC:
	...
	(f->delay_buf_pos + f->delay_buf_length - 1) % f->delay_buf_length;
      for (c = 0; c < channels; ++c) {
	double delayed_0, delayed_1, delayed_2;
	double delayed;
	double in, out;
	size_t phase = c * f->lfo_length * f->phase + .5;
	double delay = f->lfo[(f->lfo_pos + phase) % f->lfo_length];
...
"    speed-->| sine/triangle |",
"            +---------------+",
"                    | * depth    gain-out = width/100 * (1 - regen/100)",
"         ___     ___v___",
"In      |   |   |       |    * gain-out     ___",
"----+-->| + |-->| DELAY |--+-------------->|   |",
"    |   |___|   |_______|  |               |   | / (1 + gain_out)",
"    |     ^                |               | + |------------------>",
"    |     |  * regen/100   |               |   |                Out",
"    |     +----------------+               |   |",
"    +------------------------------------->|___|",
